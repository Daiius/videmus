...
export const postMediasoupClientConnect = async ({
  ...
  try {
    ...
    console.log(`[MEDIA] client-connect dtlsParameters: ${JSON.stringify(dtlsParameters)}`)
    console.log(`[MEDIA] transport state before connect: ice=${streamerTransport.iceState}, dtls=${streamerTransport.dtlsState}`)
    await streamerTransport.connect({ dtlsParameters });
    console.log(`[MEDIA] transport state after connect: ice=${streamerTransport.iceState}, dtls=${streamerTransport.dtlsState}`);
