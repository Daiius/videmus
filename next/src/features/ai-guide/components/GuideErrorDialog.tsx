    ...
    <Dialog.Root open={true} onOpenChange={(open) => { if (!open) onCancel(); }}>
      <Dialog.Portal>
        ...
        <div className="fixed inset-0 flex items-center justify-center p-4">
          <Dialog.Popup
            className="w-full max-w-sm bg-gray-800 border border-gray-700 rounded-lg shadow-xl p-6"
            data-videmus-dialog="guide-error"
          >
            <div className="flex items-start gap-3 mb-4">
              <ExclamationCircleIcon className="w-6 h-6 text-red-400 flex-shrink-0" />
              <div>
                <Dialog.Title className="font-semibold text-sm text-gray-100">
